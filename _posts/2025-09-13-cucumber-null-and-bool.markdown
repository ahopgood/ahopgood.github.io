...
Instead:
1. We'd have to make each description text distinct to satisfy Cucumber
2. Each step would need separate but similar code to parse the field
3. Nulls still wouldn't be supported
...
```
* When comparing the strings we ignore uppercase/lowercase, well because just in case ;-) 
...
```
This covers our third and final scenario:
  * > the SAML profile database field: enabled is: true
